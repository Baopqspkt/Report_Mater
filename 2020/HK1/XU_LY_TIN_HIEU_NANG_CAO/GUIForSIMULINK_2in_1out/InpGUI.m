...
% Last Modified by GUIDE v2.5 24-Nov-2020 23:17:19
...
if str2num(val) %if number is stored in edit field
    ud.editfield = str2num(val);
else % if its not number
    ud.editfield = 0; % store 0 
end
...
ud =get(gcbf,'UserData'); % get ud stored in fig userdata field
set(hObject,'String', num2str(ud.editfield)) ; %write value in userdata to field
...
function edtxt2_Callback(hObject, eventdata, handles)
% hObject    handle to edtxt2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edtxt2 as text
%        str2double(get(hObject,'String')) returns contents of edtxt2 as a double
val = get(hObject,'String'); %get the string in edit field

ud =get(gcbf,'UserData'); % get ud stored in fig userdata field
if str2num(val) %if number is stored in edit field
    ud.edit2field = str2num(val);
else % if its not number
    ud.edit2field = 0; % store 0 
end
set(gcbf,'UserData',ud) ; %update userdata

% --- Executes during object creation, after setting all properties.
function edtxt2_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edtxt2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%-----part2-2---------------% display initial value
ud =get(gcbf,'UserData'); % get ud stored in fig userdata field
set(hObject,'String', num2str(ud.edit2field)) ; %write value in userdata to field
guidata(hObject, handles); %update view
%-----part2-2 ends----------------%
